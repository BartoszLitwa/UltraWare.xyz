...
//--------------------------------------------------------------------------------
void Visuals::Player::RenderBackTrackTicks()
{
	if(g_Options.BackTrack_BackTrack && BackTrack::records[ctx.pl->EntIndex()].size() >= 3)
		for (int Tick = 0; Tick < BackTrack::records[ctx.pl->EntIndex()].size() - 1; Tick++) {
			Vector HeadTickpos;
...
//--------------------------------------------------------------------------------
void Visuals::Player::RenderSkeleton() //https://www.unknowncheats.me/forum/counterstrike-global-offensive/261581-appealing-bone-esp.html - better skeleton esp
{
	...
	if (ctx.pl->SetupBones(BoneToWorld, MAXSTUDIOBONES, 256, g_GlobalVars->curtime)) {
		for (int i = 0; i < studiohdr->numbones; i++) {
			mstudiobone_t* Bone = studiohdr->GetBone(i);
			if (Bone && (Bone->flags & BONE_USED_BY_HITBOX) && (Bone->parent != -1)) {
				auto vChild = ctx.pl->GetBonePos(i);
				auto vParent = ctx.pl->GetBonePos(Bone->parent);

				int ChestBone = 6; // Parameter of relevant Bone number - Chest
				Vector vBreastBone; // New reference Point for connecting many bones
				Vector vUpperDirection = ctx.pl->GetBonePos(ChestBone + 1) - ctx.pl->GetBonePos(ChestBone); // direction vector from chest to neck
				vBreastBone = ctx.pl->GetBonePos(ChestBone) + vUpperDirection / 2;
				Vector vDeltaChild = vChild - vBreastBone; // Used to determine close bones to the refernce point
				...
				if (abs(vDeltaChild.z) < 5 && (vDeltaParent.Size() < 5 && vDeltaChild.Size() < 5) || i == ChestBone)
					continue;
				...
				Render::Get().RenderLine(sParent[0], sParent[1], sChild[0], sChild[1], Color(255, 255, 255));
			}
		}
	}
}
...
void Visuals::Player::RenderAmmo()
{
	...
	auto MaxAmmo = weapon->GetCSWeaponData()->iMaxClip1;
	auto Ammo = weapon->GetCSWeaponData()->iBullets;

	//auto text = weapon->GetCSWeaponData()->szWeaponName + 7;
	//auto sz = g_pDefaultFont->CalcTextSizeA(14.f, FLT_MAX, 0.0f, MaxAmmo);
	//Render::Get().RenderText(text, ctx.feet_pos.x, ctx.feet_pos.y, 14.f, ctx.clr, true,g_pDefaultFont);
	//Render::Get().RenderBox(ctx.bbox.left, ctx.bbox.bottom + 4, (float)fabs(ctx.bbox.left - ctx.bbox.right), ctx.bbox.bottom + 8, Color::White, 1.f, true);
}
//--------------------------------------------------------------------------------
...
void Visuals::RenderPlantedC4(C_BaseEntity* ent)
{
	...
	auto name = (bombTimer < 0.f) ? "Bomb" : "Bomb: " + timer;
	auto sz = g_pDefaultFont->CalcTextSizeA(14.f, FLT_MAX, 0.0f, name.c_str());
	int w = bbox.right - bbox.left;
	if (bombTimer < 1)
		bombTimer = 1;
	Color C4UpColor = Color(0.f, 255.f, 0.f);
	if (bombTimer < 11)
		C4UpColor = Color(255.f, 255.f, 0.f);
	if (bombTimer < 6)
		C4UpColor = Color(255.f, 0.f, 0.f);

	Render::Get().RenderText(name, ImVec2((bbox.left + w * 0.5f) - sz.x * 0.5f, bbox.bottom + 1), 14.f, g_Options.color_esp_c4);
	Render::Get().RenderText(name, ImVec2(10, 10), 30, C4UpColor);
}
//--------------------------------------------------------------------------------
...
void Visuals::ThirdPerson() {
	...
	static bool ThirdpersonSet = false;
	if (!GetKeyState(VK_F1))
		ThirdpersonSet = false;
	else 
		ThirdpersonSet = true;

	if (g_Options.misc_thirdperson && g_LocalPlayer->IsAlive() && ThirdpersonSet)
	{
...
void Visuals::AddToDrawList() {
	for (auto i = 1; i <= g_EntityList->GetHighestEntityIndex(); ++i) {
		...
		if (g_Options.esp_localplayer) {
			if (entity == g_LocalPlayer && !g_Input->m_fCameraInThirdPerson)
				continue;
		}
		else if(!g_Options.esp_localplayer && entity == g_LocalPlayer)
			continue;
		...
		if (i <= g_GlobalVars->maxClients) {
			...
			if (player.Begin((C_BasePlayer*)entity)) {
				...
				if (g_Options.esp_player_BackTrackTicks)    player.RenderBackTrackTicks();
			}
		}
