...
namespace Utils {
	...
	template <typename T>
	static auto relativeToAbsolute(int* address) noexcept
	{
		return reinterpret_cast<T>(reinterpret_cast<char*>(address + 1) + *address);
	}

	bool lineGoesThroughSmoke(Vector source, Vector destiantion, short index) {
		static auto goThrough = (bool(__fastcall*)(Vector, Vector, short))(PatternScan(GetModuleHandleW(L"client_panorama"), "E8 ?? ?? 8B 4C 24 30 33 D2" + 1));
		return goThrough(source, destiantion, index);
	}
		
	//smokeCount = *(reinterpret_cast<int**>(lineGoesThroughSmoke) + 2);

	void setAbsOrigin(C_BasePlayer* entity, const Vector& origin)
	{
		static auto setabsorigin = (int(__fastcall*)(C_BasePlayer*, const Vector&))(PatternScan(GetModuleHandleW(L"client_panorama"), "E8 ?? ?? EB 19 8B 07"+ 1));
		setabsorigin(entity, origin);
	}
